...
# Add cooldown tracking for each class
class_cooldowns = {cls: 0 for cls in original_classes}
COOLDOWN_PERIOD = 8.0  # 8 seconds cooldown per class

# Add a flag to track if Arduino is busy
arduino_busy = False
arduino_last_command_time = 0
...
while True:
    ...
    result = model(frame, imgsz=320, conf=0.85, verbose=False)
    ...
    # Check if enough time has passed since the last Arduino command
    if current_time - arduino_last_command_time < COOLDOWN_PERIOD:
        arduino_busy = True
    else:
        arduino_busy = False
    ...
    if detections and detections.xyxy is not None and len(detections.xyxy) > 0:
        for i, box in enumerate(detections.xyxy):
            ...
            if rect_x1 <= cx <= rect_x2 and rect_y1 <= cy <= rect_y2:
                ...
                # Only process new detections if Arduino is not busy and cooldown period has passed
                if not arduino_busy and current_time - class_cooldowns[class_name] > COOLDOWN_PERIOD:
                    counts[class_name] += 1
                    class_cooldowns[class_name] = current_time
                    arduino_last_command_time = current_time
                ...
                color = (0, 255, 0) if not arduino_busy else (0, 0, 255)  # Green if ready, Red if busy
    ...
    status_text = "READY" if not arduino_busy else "BUSY"
    status_color = (0, 255, 0) if not arduino_busy else (0, 0, 255)
    cv2.putText(annotated_frame, f"Status: {status_text}", (10, annotated_frame.shape[0] - 20),
